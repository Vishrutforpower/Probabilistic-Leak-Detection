...
def main():
    ...
    while grid[leakrow][leakcol].status != 'Bot':
        ...
        #OPEN CELLS IN SQUARE LOOKS AT ALL THE OPEN CELLS WITHIN THE DETECTION SQUARE AND CAN DETECT
        #IF LEAK EXISTS OR NOT
        opencellsinsquare = []
        for i in range(minX, maxX):
            for j in range(minY, maxY):
                ...
                if grid[i][j].leak == True:
                    detection = True
                    opencellsinsquare.append([i, j])
        #TOTAL ACTIONS INCREMENTED BY ONE
        total_actions += 1
        #IF DETECTION IN FALSE, WE TAKE OUT ALL CELLS IN OPEN CELLS IN SQUARE FROM MAY CONTAIN LEAK
        if detection == False:
            may_contain_leak = [i for i in may_contain_leak if i not in opencellsinsquare]
        #MAY CONTAIN LEAK TAKES ALL THE CELLS FROM OPEN CELLS IN SQUARE IF DETECTION SQAURE DETECTED A LEAK
        if detection == True:
            may_contain_leak = opencellsinsquare
        distance = math.sqrt(math.pow(rob.r - may_contain_leak[0][0], 2) + math.pow(rob.c - may_contain_leak[0][1], 2))
        #SHORTEST DISTANCE FROM BOT CELL IS TRACKED, I DID NOT USE BFS OVER HERE BUT USED IN LATER BOTS
        for i in may_contain_leak:
            var = math.sqrt(math.pow(rob.r - i[0], 2) + math.pow(rob.c - i[1], 2))
            if var > distance and var != 0:
                distance = var
                rob.r = i[0]
                rob.c = i[1]
        grid[rob.r][rob.c].status = 'Bot'
        total_actions += distance
        grid[leakrow][leakcol].status = 'Bot'  
        
        
    
    
    #TOTAL ACTIONS IT TOOK UNTIL THE BOT TO REACH THE LEAK
